...
class Companion {
  ...
  diveAttack() {
    ...
    spawnText(this.x - camX, this.gy - this.flyH - 30, TR('loroDive') || '🦜 MERGULHO!', '#66ff88', 1.1);
    ...
    if (this.diveTarget) {
      ...
      if (alvo.dead || alvo.removeMe) { this.diveTarget = null; }
        ...
        if (Math.abs(tdx) < 42 && Math.abs(tdy) < 26) {
          ...
          spawnText(alvo.screenX, alvo.screenY - 100, fmt(TR('loroBite') || '🦜 BICADA! ({n})', dmg), '#66ff88', 1.1);
          this.echoFlash = 0.4;
          sfx.hit();
          this.diveTarget = null;
        }
  ...
  onPlayerHit(target) {
    ...
    const eco = TR('loroEcho') || ['🦜 alucinou! ({n})', '🦜 repetiu! ({n})', '🦜 REPETIU CRITADO! ({n})'];
    ...
    if (this.target) {
      ...
      if (alvo.dead || alvo.removeMe) { this.target = null; }
        ...
        if (Math.abs(tdx) < 44 && Math.abs(tdy) < 24) {
          ...
          alvo.takeHit(dmg, this.facing, false);
          spawnText(alvo.screenX, alvo.screenY - 90, fmt(TR('saciHit') || '🌪 PANCADA! ({n})', dmg), '#ff5533', 1.1);
          this.hitFlash = 0.4;
          sfx.hit();
          this.target = null;
        }
...
class OptimusBot extends Lobista {
  ...
  demitir() { // o Ilon corta custos NA LUTA
    if (this.dead) return;
    this.dead = true; this.frame = 0; this.frameTime = 0;
    spawnText(this.screenX, this.screenY - 100, TR('demitido') || '📉 DEMITIDO!', '#ff8866', 1.1);
    sfx.enemyDie();
  }
}
...
class CloneTemu extends Lobista {
  constructor(x, gy) {
    ...
    this.label = TR('cloneTemuLabel') || 'VOCÊ (versão temu)';
    this.scoreValue = 180;
  }
  draw() {
    ctx.save();
...
class Trunfo extends Entity {
  ...
  update() {
    ...
    if (lostQuarters > this.rageLevel) {
      ...
      const flipPrefix = TR('trunfoFlipPrefix') || '💢 MUDEI DE OPINIÃO: ';
    ...
    if (this.talkCd <= 0) {
      ...
      const falas = TR('trunfo') || ['FAKE NEWS!', 'TARIFA NELES!', 'Ninguém sabe mais de AGI que eu!', 'Vou construir um MURO de firewall!', 'Isso é caça às bruxas!'];
    ...
    if (this.state === 'sign') {
      ...
      if (this.frame === 3 && !this.hitDone) {
        ...
        spawnText(this.screenX, this.screenY - 130, TR('canetada') || '✍ CANETADA!', '#ffd23f', 1.2);
      }
...
class Ilon extends Entity {
  ...
  update() {
    ...
    if (lostQuarters > this.rageLevel) {
      this.rageLevel = lostQuarters;
      spawnText(this.screenX, this.screenY - 170, TR('ilonCorte') || '📉 CORTE DE CUSTOS! TODO MUNDO DEMITIDO!', '#ff6644', 1.3);
    ...
    if (this.talkCd <= 0) {
      this.talkCd = 5 + Math.random() * 3;
      const falas = TR('ilon') || ['Trabalhe 80 horas ou saia!', 'Isso é só o protótipo do meu plano!', 'Vou tuitar sobre essa luta!', 'Renda básica pra você. O resto pra mim.', 'Comprei essa fase. Agora é minha.'];
    ...
    if (this.state === 'tweet') {
      ...
      if (this.frame === 3 && !this.hitDone) {
        ...
        spawnText(this.screenX, this.screenY - 130, TR('ilonTweet') || '📱 TUITADA!', '#88ddff', 1.2);
      }
    ...
    if (this.state === 'command') {
      ...
      if (this.frame === 3 && !this.hitDone) {
        this.hitDone = true;
        spawnText(this.screenX, this.screenY - 150, TR('ilonRobots') || 'ROBÔS: PRODUTIVIDADE MÁXIMA!', '#ffaa55', 1.2);
...
class Samuca extends Entity {
  ...
  update() {
    ...
    if (this.talkCd <= 0) {
      this.talkCd = 5 + Math.random() * 3;
      const falas = TR('samuca') || ['Feedback é bem-vindo!', 'Isso é uma experiência antecipada!', 'Assina aqui, aqui e aqui.', 'AGI ano que vem. Confia.', 'Sua derrota é beneficial for humanity*'];
    ...
    if (this.state === 'box') {
      ...
      if (this.frame === 3 && !this.hitDone) {
        ...
        spawnText(this.screenX, this.screenY - 130, TR('samucaBox') || '📦 PRODUTO NOVO!', '#aaffcc', 1.2);
      }
    ...
    if (this.state === 'launch') {
      ...
      if (this.frame === 3 && !this.hitDone) {
        ...
        if (Math.random() < 0.3) {
          spawnText(this.screenX, this.screenY - 170, TR('samucaFail') || '💥 A DEMO TRAVOU NA FRENTE DE TODO MUNDO!', '#ff6644', 1.3);
          this.stunned = 2.5;
          sfx.hurt();
        } else {
          const nome = LANCAMENTOS[Math.floor(Math.random() * LANCAMENTOS.length)];
          spawnText(this.screenX, this.screenY - 170, fmt(TR('samucaLaunch') || '🚀 LANÇAMENTO: {n}!', nome), '#ffd23f', 1.3);
...
class Dario extends Entity {
  ...
  update() {
    ...
    if (lostQuarters > this.rageLevel) {
      this.rageLevel = lostQuarters;
      spawnText(this.screenX, this.screenY - 170, TR('darioRage') || '😇 "Isso é pela segurança de TODOS."', '#ccccff', 1.2);
      this.attackCd = 0.8;
    }
    this.talkCd -= dt;
    if (this.talkCd <= 0) {
      this.talkCd = 5 + Math.random() * 3;
      const falas = TR('dario') || ['Seus dados já são meus.', 'Vou escrever 15 mil palavras sobre essa luta.', 'Minha mãe entendeu. Você vai entender.', 'É pra sua segurança. Confia.', 'Interessante... *anota tudo*'];
    ...
    if (this.state === 'vacuum') {
      ...
      if (Math.abs(ldx) < range) {
        ...
        if (Math.abs(companion.x - this.x) < 90) {
          ...
          spawnText(this.screenX, this.screenY - 170, TR('darioChoke') || '🤢 ENGASGOU COM DADO SINTÉTICO!', '#66ff88', 1.3);
          spawnText(companion.x - camX, companion.gy - 130, TR('loroChoke') || '🦜 me GEROU não, me ENGOLIU não!', '#66ff88', 1.1);
          sfx.hurt(); shake = 0.25;
          return;
        }
      }
    ...
    if (this.state === 'essay') {
      ...
      if (this.frame === 3 && !this.hitDone) {
        this.hitDone = true;
        spawnText(this.screenX, this.screenY - 170, TR('darioEssay') || '📜 ENSAIO: "MÁQUINAS DA GRAÇA AMOROSA" (15.000 palavras)', '#ccccff', 1.2);
    ...
    if (this.attackCd <= 0) {
      ...
      this.biteDone = false;
      spawnText(this.screenX, this.screenY - 150, TR('darioVacuum') || '🌀 MODO ASPIRADOR!', '#ccccff', 1.2);
      return;
    }
    const wantDist = 260;
    const dist = Math.abs(dx);
...
class DeepZeek extends Entity {
  ...
  update() {
    ...
    if (!this.openSourced && this.hp <= this.maxHp / 2) {
      ...
      spawnText(this.screenX, this.screenY - 200, TR('deepzeekOpen') || '🐉 RELEASE OPEN-SOURCE! PESOS LIBERADOS!', '#88ffcc', 1.4);
    ...
    if (this.talkCd <= 0) {
      this.talkCd = 5 + Math.random() * 3;
      const falas = TR('deepzeek') || ['Metade do orçamento. Dobro do resultado.', '*escaneia* — golpe catalogado.', 'Querem chá?', 'Seu itinerário eu já conhecia há 3 semanas.', 'Ineficiente. Mas corajoso.'];
    ...
    if (this.state === 'clone') {
      ...
      if (this.frame === 3 && !this.hitDone) {
        this.hitDone = true;
        spawnText(this.screenX, this.screenY - 180, TR('deepzeekClone') || '⚡ CLONADO. CUSTO: 1/10.', '#88ffcc', 1.2);
    ...
    if (this.state === 'breath') {
      ...
      if (this.frame === 3 && !this.hitDone) {
        ...
        spawnText(this.screenX, this.screenY - 170, TR('deepzeekBreath') || '🔥 SOPRO BINÁRIO!', '#ff8888', 1.2);
      }
      if (done) { this.setState('idle'); this.attackCd = Math.max(1.2, 2.4 - this.rageLevel * 0.3); }
      return;
    }
...
class Gemeo extends Entity {
  ...
  update() {
    ...
    if (lostQuarters > this.rageLevel) {
      this.rageLevel = lostQuarters;
      spawnText(this.screenX, this.screenY - 160, TR('gemeoTele') || '✨ REALINHAMENTO ÓPTICO!', '#cc88ff', 1.3);
    ...
    if (this.talkCd <= 0) {
      this.talkCd = 5 + Math.random() * 3;
      const falas = TR('gemeo') || ['DEDOS. OLEOSOS.', 'UM ESPIRRO DESALINHA TUDO.', 'PRECISÃO É AMOR.', 'VOCÊS TREMEM DEMAIS.'];
    ...
    if (this.state === 'beam') {
      ...
      if (this.frame === 3 && !this.hitDone) {
        ...
        spawnText(this.screenX, this.screenY - 130, TR('gemeoBeam') || '💜 FEIXE UV!', '#cc88ff', 1.2);
      }
...
function drawDeepZeekCamarote() {
  ...
  if (Math.floor(time / 6) % 3 === 0 && (time % 6) < 3) {
    ...
    ctx.fillText(TR('aprovaSilencio') || '*aprova em silêncio*', cx, cy - 60);
  }
}
  ...
  if (gameState === 'menu') {
    ...
    if (currentPhase.finalPhase) {
      ...
      if (treino >= TRAIN_TIME && !awaken) {
        awaken = true;
        spawnText(W / 2, H / 2 - 60, TR('curupiraOnline') || '⚡ CURUPIRA-1 ONLINE ⚡', '#66ff88', 2);
